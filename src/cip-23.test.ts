...
describe('getTypeHash', () => {
  it('returns a 32 byte hash for a type', () => {
    expect(bytesToHex(getTypeHash(mailTypedData, 'CIP23Domain'))).toBe(
    ...
    expect(bytesToHex(getTypeHash(approvalTypedData, 'CIP23Domain'))).toBe(
    ...
    expect(bytesToHex(getTypeHash(arrayTypedData, 'CIP23Domain'))).toBe(
      'f84835081e4bf7951170279938141a886e3f74e3ad5baf55f0aa17d764ab3192'
    );
  ...
  it('throws for invalid JSON data', () => {
    // @ts-expect-error type is missing `CIP23Domain`
    expect(() => bytesToHex(getTypeHash(invalidSchema, 'CIP23Domain'))).toThrow();
  });
});

describe('encodeData', () => {
  it('encodes data to an ABI encoded string', () => {
    expect(bytesToHex(encodeData(mailTypedData, 'CIP23Domain', mailTypedData.domain))).toBe(
    ...
    expect(bytesToHex(encodeData(approvalTypedData, 'CIP23Domain', approvalTypedData.domain))).toBe(
  ...
  it('throws for invalid JSON data', () => {
    // @ts-expect-error type is missing `CIP23Domain`
    expect(() => bytesToHex(encodeData(invalidSchema, 'CIP23Domain', invalidSchema.domain))).toThrow();
  });

  /* it('throws when a type is missing', () => {
    expect(() => bytesToHex(encodeData(invalidMissingData, 'Mail', invalidMissingData.message)).toThrow();
  }); */

  it('throws when data is missing', () => {
    expect(() => bytesToHex(encodeData(invalidMissingType, 'Mail', invalidMissingType.message))).toThrow();
  });

  it('throws if the type is not an array', () => {
    expect(() => bytesToHex(encodeData(invalidArrayType, 'Mail', invalidArrayType.message))).toThrow();
  });

  it('throws if the array length is invalid', () => {
    expect(() => bytesToHex(encodeData(invalidArrayLength, 'Mail', invalidArrayLength.message))).toThrow();
  });
});

describe('getStructHash', () => {
  it('returns a 32 byte hash for a struct', () => {
    expect(bytesToHex(getStructHash(mailTypedData, 'CIP23Domain', mailTypedData.domain))).toBe(
    ...
    expect(bytesToHex(getStructHash(approvalTypedData, 'CIP23Domain', approvalTypedData.domain))).toBe(
  ...
  it('throws when a type is missing', () => {
    expect(() => bytesToHex(encodeData(invalidMissingType, 'Mail', invalidSchema.message))).toThrow();
  });
...
describe('getMessage', () => {
  it('returns the full encoded and hashed message to sign', () => {
    ...
    expect(bytesToHex(getMessage(mailTypedDataWithRecursiveTypes))).toBe(
  ...
  it('throws for invalid JSON data', () => {
    // @ts-expect-error type is missing `CIP23Domain`
    expect(() => bytesToHex(getMessage(invalidSchema))).toThrow();
  });

  it('throws when a type is missing', () => {
    expect(() => bytesToHex(getMessage(invalidMissingType))).toThrow();
  });
